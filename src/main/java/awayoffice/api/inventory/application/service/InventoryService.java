...
public class InventoryService {
    ...
    public AssetModelDTO getAssetModelById(Long id){
        ...
        if(assetModel != null){ return assetModelAssembler.toModel(assetModel); }
        else{return null;}
    }
    //========[AssetModel]========
    //@Author: Abdul
    ...
    public CollectionModel<AssetModelDTO> getAllAssetModels() throws Exception{
        ...
        if(assetModels.size() <= 0){ return null; }
        else {return assetModelAssembler.toCollectionModel(assetModels);}
    }

    //========[AssetModel]========
    //@Author: Abdul
    //Create [AssetModel-01]
    public AssetModelDTO createAssetModel(AssetModelDTO assetModelDTO) throws Exception{
    ...
    public AssetModelDTO updateAssetModel(AssetModelDTO assetModelDTO) throws Exception{
        AssetModel assetModel = assetModelRepository.findById(assetModelDTO.getId()).orElse(null);
        if (assetModel != null) {
            assetModel.setName(assetModelDTO.getName());
            assetModel.setModel(assetModelDTO.getModel());
            assetModel.setManufacturer(assetModelDTO.getManufacturer());
